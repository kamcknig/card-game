...
export const configureFerryman = (args: ExpansionConfiguratorContext) => {
  ...
  console.info(`[cornucopia configurator - configuring ferryman] ferryman present in supply`);
  ...
  if (!availableKeys.length) {
    console.info(`[cornucopia configurator - configuring ferryman] no available kingdoms, not adding new kingdom`);
    return;
  }
  
  const chosenKey = availableKeys[Math.floor(Math.random() * availableKeys.length)];
  
  console.info(`[cornucopia configurator - configuring ferryman] adding ${chosenKey} to kingdom as the "ferryman" card`);
