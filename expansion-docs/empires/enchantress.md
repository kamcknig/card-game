...
- Enchantress does not change anything about the card, it just prevents on-play instructions on the card from being carried out properly. So cards that are Enchanted can still activate Reactions (such as another player's Diplomat), still carry out all Adventures tokens effects (such as the +$1 token from Training), and can still be replayed (e.g. by Royal Carriage) to perform their normal effects.
