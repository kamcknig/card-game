...
export const configureArena = (registrar: GameEventRegistrar, config: ComputedMatchConfiguration) => {
  const hasArena = (config.landmarks ?? []).some(landmark => landmark.cardKey === 'arena');
  ...
  registrar('onGameStartSetup', async args => {
    ...
    const arenaLandmark = args.match.landmarks.find(landmark => landmark.cardKey === 'arena');
    if (!arenaLandmark) {
      return;
    }
    ...
    for (const player of args.match.players) {
      args.reactionManager.registerReactionTemplate(arenaLandmark, 'startTurnPhase', {
        playerId: player.id,
        once: false,
        ...
        condition: async conditionArgs => {
          ...
          const actionCards = conditionArgs.findCardService.findCards({
            all: [{ location: 'playerHand', playerId: player.id }, { cardType: 'ACTION' }],
          });
        ...
        triggeredEffectFn: async triggeredArgs => {
          ...
          const tokensOnArena = Object.values(triggeredArgs.match.tokens ?? {})
            .filter(
              token =>
