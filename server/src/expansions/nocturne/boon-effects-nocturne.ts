...
const registerFieldsGift = (registerBoonEffect: BoonEffectRegistrar) => {
  registerBoonEffect(
    'the-fields-gift',
    async ({ loggerService, playerId, actionService, match, reactionManager, cardId }) => {
...
const registerSeasGift = (registerBoonEffect: BoonEffectRegistrar) => {
  registerBoonEffect('the-seas-gift', async ({ playerId, actionService }) => {
    await actionService.run('drawCard', { playerId, count: 1 });
  });
};
...
const registerWindsGift = (registerBoonEffect: BoonEffectRegistrar) => {
  registerBoonEffect(
    'the-winds-gift',
    async ({ loggerService, playerId, actionService, cardLibrary, cardSourceController }) => {
