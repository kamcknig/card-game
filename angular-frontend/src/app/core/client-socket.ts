...
import { displayScene } from './scene/display-scene';
import { $gameOwner, $gamePaused } from '../state/game-state';
...
export const socketToGameEventMap: { [p in ClientListenEventNames]: ClientListenEvents[p] } = {
  ...
  playerConnected: (player) => {
    if (!$playerIds.get().includes(player.id)) {
      $player(player.id).set(player);
      $playerIds.set([...$playerIds.get(), player.id]);
    }
  ...
  setPlayerList: players => {
    ...
    $playerIds.set(players.map(player => player.id));
  },
