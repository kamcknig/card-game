...
- The played Action card stays in the Supply; if an effect tries to move it, such as Encampment trying to set itself aside, it will fail to move it.
- Since the played card is not in play, "while this is in play" abilities (like on Groundskeeper†) will not do anything.
- Leave Overlord in play until the Clean-up of the turn the card it played would have left play. Normally that means you'll discard it that turn, but if Overlord plays Crown which in turn plays a Duration card several times, Overlord will stay out with the Duration card as Crown would have.
...
- Card Groundskeeper is planned to get reprinted with its while-in-play timer removed as part of errata being implemented since 2022.
- Cards Urchin and Goons, however, may persist as valid examples.
...
- Overlord received further errata in 2025 to no longer allow it to play Duration cards.
