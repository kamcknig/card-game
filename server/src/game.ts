...
export class Game {
  ...
  public async getExpansionList() {
    ...
    try {
      this._expansionList = (await import(`./expansions/expansion-list.json`, {with: {type: 'json'}})).default;
  ...
  public addPlayer(sessionId: string, socket: AppSocket) {
    ...
    socket.on('disconnect', (arg) => this.onPlayerDisconnected(player, arg.toString()));
  }
  ...
  private onPlayerDisconnected = (player: Player, reason: string) => {
    ...
    if (player.id === this.owner?.id) {
      for (const checkPlayer of this.players.filter((p) => p.id !== player.id)) {
  ...
  private onMatchConfigurationUpdated = async (config: MatchConfiguration) => {
    ...
    for (const expansion of newExpansions) {
      ...
      for (const exclusiveExpansion of configModule.mutuallyExclusiveExpansions) {
        if (config.expansions.includes(exclusiveExpansion) && !expansionsToRemove.includes(exclusiveExpansion)) {
    ...
    this._matchConfiguration = {
      ...this._matchConfiguration,
      expansions: config.expansions.filter(e => !expansionsToRemove.includes(e))
    };
  ...
  private onPlayerReady = (playerId: number) => {
    ...
    this._match = new MatchController([...sessionSocketMap.values()]);
    ...
    void this._match.initialize(config);
  };
}