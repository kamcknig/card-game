...
const gainHorse = async (
  ...
  const horseCards = args.findCardService.findCards({ all: [{ location: 'nonSupplyCards' }, { cardKeys: 'horse' }] });
...
const getActionSupplyPileKeys = (args: CardEffectFunctionContext): string[] => {
...
const isInvestedCardStillInOwnerExile = (args: AppContext, cardId: CardId, ownerPlayerId: PlayerId): boolean => {
...
const effectMap: CardExpansionModule = {
  ...
  banish: {
    registerEffects: () => async cardEffectArgs => {
      ...
      const cardIdsByKey = hand.reduce(
        (prev, cardId) => {
  ...
  commerce: {
    registerEffects: () => async cardEffectArgs => {
      ...
      for (const gainedCardId of gainedThisTurn) {
        ...
        if (
          !gainStats ||
          gainStats.playerId !== cardEffectArgs.playerId ||
      ...
      cardEffectArgs.reactionManager.registerReactionTemplate(
        ...
        'startTurn',
        {
          ...
          condition: async conditionArgs =>
            ...
            getPlayerSourceSafe(conditionArgs, 'set-aside', cardEffectArgs.playerId).includes(selectedActionCardId),
    ...
    registerEffects: () => async cardEffectArgs => {
      ...
      while (remainingPileKeys.length) {
        ...
        const selectedTopCard =
          selectableTopCards.find(entry => entry.card.id === resolvedCardId) ?? selectableTopCards[0];
